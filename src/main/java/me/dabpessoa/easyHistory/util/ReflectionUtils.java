...
/**
 * 
 * Classe que possui métodos auxiliares para localização de valores via reflection em uma
 * classe qualquer.
 * 
...
public class ReflectionUtils {
	...
	public static <T, A> boolean hasClassAnnotation(Class<T> clazz, Class<A> annotation) {
		return findClassAnnotation(clazz, annotation) != null;
	}

	public static <T, A> boolean hasAnyFieldAnnotation(Class<T> clazz, Class<A> annotation) {
		return findAnnotation(clazz, annotation) != null;
	}

	public static <T, A> Annotation findClassAnnotation(Class<T> clazz, Class<A> annotation) {
		Class<?> classe = clazz;
		while (classe != null && classe != Object.class) {
			Annotation[] as = classe.getDeclaredAnnotations();
			for (Annotation a : as) {
				if (a.annotationType().getName().equalsIgnoreCase(annotation.getName())) {
					return a;
				}
			}
	...
	public static <T> List<Field> findFieldsByAnnotations(Class<T> clazz, Class<?>... annotationClasses) {
		...
		if (annotationClasses != null) {
			for (Class<?> annotationClass : annotationClasses) {
				List<Field> findedFields = findFieldsByAnnotation(clazz, annotationClass);
				if (findedFields != null) {
					for (Field findedField : findedFields) {
